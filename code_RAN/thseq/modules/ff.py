...
class DeepTransitionLayer(nn.Module):
    def __init__(self, inpt_dim, tran_size, dropout=0.0):
        super(DeepTransitionLayer, self).__init__()

        self.trans_linear1 = nn.Linear(inpt_dim, tran_size)
        self.trans_linear2 = nn.Linear(tran_size, inpt_dim)
        self.drop = nn.Dropout(dropout)
        self.relu = nn.ReLU()

    def forward(self, inpt):
        res = inpt
        h = self.drop(self.relu(self.trans_linear1(inpt)))
        h = self.drop(self.relu(self.trans_linear2(h)) + h)
        return h

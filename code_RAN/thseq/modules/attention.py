...
from .ff import PositionWiseFeedForward

__all__ = ['MultiHeadAttention','RandomAttention', 'RecRandomAttention',
           'MultiHeadwithRandomAttention', 'MultiHeadwithRecRandomAttention',
...
class FactorizedRandomAttention(nn.Module):
    def __init__(self,
        ...
        maxlen = 256
        k = 8
        self.randatt1 = nn.Parameter(torch.zeros(1, num_heads, maxlen, k))
        self.randatt2 = nn.Linear(k, maxlen, bias=False)
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.randatt1)
        nn.init.xavier_uniform_(self.randatt2.weight)
    ...
    def forward(self, q, k, v, mask=None, state=None, avoid_nan=False):
        ...
        randatt = self.randatt2(self.randatt1)

        if self.is_incremental and T == 1:
            score = randatt[:, :, v.size(1)-1, :v.size(1)].unsqueeze(2).expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()
        else:
            score = randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()
...
class RandomAttention(nn.Module):
    def __init__(self,
        ...
        maxlen = 256
        self.randatt = nn.Parameter(torch.ones(1, num_heads, maxlen, maxlen))
    ...
    def forward(self, q, k, v, mask=None, state=None, avoid_nan=False):
        ...
        if self.is_incremental and T == 1:
            score = self.randatt[:, :, v.size(1)-1, :v.size(1)].unsqueeze(2).expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()
        else:
            score = self.randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()
...
class RecRandomAttention(nn.Module):
    def __init__(self,
                 hidden_size,
                 num_heads,
                 is_incremental,
    ...
    def reset_parameters(self):
        ...
        # nn.init.xavier_uniform_(self.randatt, gain=gain)
        nn.init.xavier_uniform_(self.linear_v.weight, gain=gain)
    ...
    def forward(self, q, k, v, randatt, mask=None, state=None, avoid_nan=False):
        """
        Compute multi-head attention with give q/k/v.
        Args:
            q: (T x B x d_q)
        ...
        # score = randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1)
        # dense_score = self.score_ffn(v).transpose(1,2)
...
class MultiHeadwithRandomAttention(nn.Module):
    def __init__(self, hidden_size, num_heads,
                 is_incremental,
                 q_size=None, k_size=None, v_size=None,
                 output_size=None,
        ...
        maxlen = 256
        self.randatt = nn.Parameter(torch.zeros(1, num_heads, maxlen, maxlen))
    ...
    def forward(self, q, k, v, mask=None, state=None, avoid_nan=False):
        ...
        if self.is_incremental and T == 1:
            syn_score = self.randatt[:, :, v.size(1)-1, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()
        else:
            syn_score = self.randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()


        # syn_score = self.randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1)
        score = score + syn_score
...
class MultiHeadwithRecRandomAttention(nn.Module):
    def __init__(self, hidden_size, num_heads,
                 is_incremental,
                 q_size=None, k_size=None, v_size=None,
                 output_size=None,
    ...
    def forward(self, q, k, v, randatt, mask=None, state=None, avoid_nan=False):
        ...
        if self.is_incremental and T == 1:
            syn_score = randatt[:, :, v.size(1) - 1, :v.size(1)].expand(B, num_head, T, -1).reshape(
                B * self.num_heads, T, -1).clone()
        else:
            syn_score = randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1).clone()

        # syn_score = randatt[:, :, :T, :v.size(1)].expand(B, num_head, T, -1).reshape(B * self.num_heads, T, -1)
        score = score + syn_score
